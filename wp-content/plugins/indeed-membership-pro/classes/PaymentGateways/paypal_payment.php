  ...
  $levels = \Indeed\Ihc\Db\Memberships::getAll();
	...
	if (ihc_is_magic_feat_active('level_dynamic_price') && isset($_GET['ihc_dynamic_price'])){
		...
		if (ihc_check_dynamic_price_from_user($_GET['lid'], $temp_amount)){
			...
			Ihc_User_Logs::write_log( esc_html__('PayPal Payment: Dynamic price on - Amount is set by the user @ ', 'ihc') . $level_arr['price'] . $currency, 'payments');
		}
	}
	/**************************** DYNAMIC PRICE ***************************/
	...
	if (!empty($_GET['ihc_coupon'])){
		...
		Ihc_User_Logs::write_log( esc_html__('PayPal Payment: the user used the following coupon: ', 'ihc') . $_GET['ihc_coupon'], 'payments');
	}
	...
	if ($reccurrence){
		...
		if ($taxes_price && !empty($taxes_price['total'])){
			...
			Ihc_User_Logs::write_log( esc_html__('PayPal Payment: taxes value set @ ', 'ihc') . $taxes_price['total'] . $currency, 'payments');
		}

		$q .= 'a3=' . urlencode($level_arr['price']) . '&';
		Ihc_User_Logs::write_log( esc_html__('PayPal Payment: amount set @ ', 'ihc') . $level_arr['price'] . $currency, 'payments');
		...
		if ($taxes_price && !empty($taxes_price['total'])){
			$level_arr['price'] += $taxes_price['total'];
			Ihc_User_Logs::write_log( esc_html__('PayPal Payment: taxes value: ', 'ihc') . $taxes_price['total'] . $currency, 'payments');
		}
