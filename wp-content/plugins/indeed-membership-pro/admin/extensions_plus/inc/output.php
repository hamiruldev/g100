<?php
wp_enqueue_style( 'ihc-extensions-plus-css', $this->dir_url . 'assets/css/style.css' );
wp_enqueue_script( 'ihc-extensions-plus', $this->dir_url . '/files/admin.js', [], 1.1 );
wp_enqueue_style( 'ihc-font-awesome', $this->dir_url . 'assets/css/font-awesome.css' );
?>
<div class="indeed-addons-main-title">
	...
	<div class="indeed-addons-main-title-right">
		<form method="post"  id="indeed_form">
			<input type="hidden" name="do_update" value="1"/>
			<div class="ihc-js-admin-submit-indeed-form" ><i class="fa-ifs ifs-refresh"></i> Refresh</div>
		</form>
	</div>
		...
		<?php
			if (!empty($cats) && is_array($cats) && count($cats)){
				foreach ($cats as $cat){
					...
					if (empty($cat['slug']) || empty($cat['label'])){
						 continue;
					}
					?>
...
<?php
	...
	if (!empty($cats) && is_array($cats) && count($cats)){
		foreach ($cats as $cat){
			$cat = array_merge($this->return_default_array_key('cats'), $cat);
			if (empty($cat['slug']) || empty($cat['description'])){
				 continue;
			}
			?>
				<p class="indeed-description indeed-display-none" id="indeed_<?php echo $cat['slug'];?>_description"><?php echo stripslashes($cat['description']);?></p>
			<?php
		}
	}
?>
			  ...
			  <div class="indeed-item">
				...
				<div class="indeed-item-inner">
					...
					<?php if (!empty($item['author_name'])){?>
						<div class="indeed-addon-author"><a href="<?php if (!empty($item['author_link'])){
							 echo $item['author_link'];
						}else{
							echo '#';
							}?>" target="_blank">by <?php echo $item['author_name']?></a></div>
					<?php }?>
					...
					<?php
						...
						if (!empty($item['short_description'])){
							...
							elseif(!empty($item['long_description'])){
								$item_desc = $item['long_description'];
							}
					?>
					...
					<div class="indeed-addon-demo-link">
						<?php if ($installed){?>
						<div class="button button-secondary disabled">Installed</div>
						<?php } else { ?>
				...
				<div class="indeed-addon-bottom-side">
					<div class="ind-side-left">
						<div class="indeed-addon-purchase-button"><a href="<?php if (!empty($item['envato_link'])){
							 echo $item['envato_link'];
						}else{
							 echo '#';
						}?>"><span>Get it now</span>
						<?php if (isset($item['price'])){
							 echo 'from $' . $item['price'];
						}?></a></div>
					</div>
